...
if FIT_PREDICTOR
    ...
    % Guardem el predictor
    save('predictor_eyes_hog_svm.mat', 'predictor');
end

% Carreguem el predictor
load('predictor_eyes_hog_svm.mat');
...
If = insertShape(I_test,'Rectangle', [best_position, mida_imatge_crop_y, mida_imatge_crop_x]);
if best_value > 0
    % afegim un marker vermell al centroide de l'objecte
    If = insertMarker(uint8(If), best_position, 'x', 'color', 'red', 'size', 10);
